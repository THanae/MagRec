...
def plot_period(orbiter, spacecraft: int = 2, planet: str = 'Mercury'):
    """
    :param orbiter: generated by kernel_loader
    :param spacecraft: 1 or 2 for Helios 1 or 2
    :param planet: Mercury, Venus or Earth
    :return:
    """
    quantity_support()
    fig = plt.figure()
    ...
    orbiter_planet = get_planet_orbit(planet, datetime.strftime(orbiter.times[0], '%d/%m/%Y'), datetime.strftime(orbiter.times[-1], '%d/%m/%Y'))
    sun_planet = np.sqrt(orbiter_planet.x ** 2 + orbiter_planet.y ** 2 + orbiter_planet.z ** 2)
    plt.plot(orbiter_planet.times, sun_planet, label=planet+' orbit')
    plt.title(
        'Orbit of Helios ' + str(spacecraft) + '  between ' + str(orbiter.times[0]) + ' and ' + str(orbiter.times[-1]))
    plt.legend()
    plt.show()
...
def plot_absolute_distances(orbiter, spacecraft: int, planet: str = 'Mercury'):
    quantity_support()
    fig = plt.figure()
    ...
    spacecraft_planet = np.sqrt(
        (orbiter.x - orbiter_planet.x) ** 2 + (orbiter.y - orbiter_planet.y) ** 2 + (orbiter.z - orbiter_planet.z)**2)
    print(len(orbiter.x), len(orbiter_planet.x))
    plt.plot(orbiter_planet.times, spacecraft_planet, label=planet +'-Spacecraft')
    plt.title('Helios ' + str(spacecraft) + '  between ' + str(orbiter.times[0]) + ' and ' + str(orbiter.times[-1]))
    plt.legend()
    plt.show()


def get_planet_orbit(planet: str, start_date: str = '20/01/1976', end_date: str = '01/10/1979', interval: float = 1):
...
def find_similar_dates(orbiter_spacecraft, orbiter_planet):
    planet_spacecraft = np.sqrt(
        (orbiter_planet.x - orbiter_spacecraft.x) ** 2 + (orbiter_planet.y - orbiter_spacecraft.y) ** 2 + (
                orbiter_planet.z - orbiter_spacecraft.z) ** 2)
    for n in range(len(planet_spacecraft)):
        if planet_spacecraft[n] <= 0.28 * u.au:
            print(orbiter_spacecraft.times[n], planet_spacecraft[n], orbiter_spacecraft.x[n], orbiter_spacecraft.y[n])
...
if __name__ == '__main__':
    ...
    plot_absolute_distances(orbiter, probe)
    # plot_period(orbiter, spacecraft=probe)
    # find_similar_dates(orbiter, get_planet_orbit('Mercury'))

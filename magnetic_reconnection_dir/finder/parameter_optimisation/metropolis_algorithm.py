...
def metropolis_algorithm(maximum_iterations: int = 100) -> List[Union[float, dict]]:
    """
    Finds the best possible parameters by randomly generating parameters
    Moves through the parameter hyperspace with the metropolis algorithm
    :param maximum_iterations: maximum number of iterations before the code stops running, should take into account
    burn-in period
    ...
    while current_iteration < maximum_iterations and mcc[0] < 0.9:
        ...
        if challenging_mcc[0] > max_mcc[0]:
            ...
            current_parameters = metropolis(previous_parameters, current_parameters, max_mcc[0], challenging_mcc[0])
...
def metropolis(best_parameters: dict, worse_parameters: dict, best_mcc: float, worse_mcc: float):
    """
    Determines whether the center of the hypersphere moves or not following the probability worse_mcc/best_mcc
    :param best_parameters: parameters with best mcc
    :param worse_parameters: parameters with worse mcc
    :param best_mcc: best mcc
    :param worse_mcc: worse mcc
    :return: parameters to be used
    """
    try:
        switch_probability = worse_mcc / best_mcc
    except ZeroDivisionError:
        return worse_parameters
    if np.random.uniform(0, 1) <= switch_probability:
        parameters = worse_parameters
    else:
...
if __name__ == '__main__':
    mcc1 = metropolis_algorithm(maximum_iterations=100)
